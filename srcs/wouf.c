...
/*   Updated: 2020/03/09 13:39:08 by pohl             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		get_hit_info(int face, double vdir, double distance, double pos, t_object *object)
{
	...
	object->face = face;
	hit_location = distance / fabs(vdir)
...
void		ray(t_config *cfg, double angle)
{
	...
	vdir.x = 1 / cos(angle);
	vdir.y = 1 / sin(angle);
	ray_p.x = (int)cfg->pl_pos.x;
	ray_p.y = (int)cfg->pl_pos.y;
	...
	while (cfg->map[ray_p.y][ray_p.x] != 1)
	{
		if (distance.x < distance.y)
		{
			...
			if (cfg->map[ray_p.y][ray_p.x] == 1 && cfg->list->size < MAX_OBJ)
				get_hit_info((vdir.x > 0) ? 2 : 0, vdir.y, distance.x, cfg->pl_pos.y, &cfg->list->obj[cfg->list->size++]);
		...
		else
		{
			...
			if (cfg->map[ray_p.y][ray_p.x] == 1 && cfg->list->size < MAX_OBJ)
				get_hit_info((vdir.y > 0) ? 3 : 1, vdir.x, distance.y, cfg->pl_pos.x, &cfg->list->obj[cfg->list->size++]);
			distance.y += fabs(vdir.y);
		}
		if (cfg->map[ray_p.y][ray_p.x] > 1 && cfg->list->size < MAX_OBJ)
			get_sprite_info(cfg, ray_p, cfg->map[ray_p.y][ray_p.x], angle);
	}
}
